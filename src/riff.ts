...
export class ListChunk extends Chunk {
    ...
    readNext() {
        ...
        if (!this.#skipJunk || nextChunkType !== "JUNK") {
            let newChunk = new (getConstructorForId(nextChunkType))(this.buffer.subarray(this._reader.position - 8, this._reader.position + nextChunkSize), this.#skipJunk);
...
function registerListParser(listType: string, listParser: typeof DefaultListReader) {
...
export const chunkRegistration = {current: constructors, register: registerChunk, unregister: unregisterChunk};
export const listRegistration = {current: listParsers, register: registerListParser, unregister: unregisterListParser};
